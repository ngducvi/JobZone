  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(responseCompany.data.companies[0]);

        // get total jobs
        const responseJobs = await authAPI().get(recruiterApis.getDashboardStats);
        ...
        setTotalApplications7DaysAgo(responseJobs.data.totalApplications7DaysAgo);

        // Lấy thống kê
        const statsResponse = await authAPI().get(recruiterApis.getDashboardStats);
        ...
        <div className={cx('user-header')}>
          ...
          <div className={cx('user-details')}>
            ...
            <div className={cx('user-contact')}>
              <span><i className="fa-regular fa-envelope"></i> {recruiter?.email}</span>
              <span><i className="fa-solid fa-phone"></i> {recruiter?.phone || "Chưa cập nhật"}</span>
            </div>
          </div>
          <div className={cx('membership-info')}>
            <div className={cx('membership-level')}>
              <span style={{ color: planDetails[recruiter?.plan]?.color }}>
                <FontAwesomeIcon icon={planDetails[recruiter?.plan]?.icon} /> Gói: {recruiter?.plan || "Chưa có gói nào"}
              </span>
            ...
            <div className={cx('membership-points')}>
              ...
              <div className={cx('progress-bar')}>
                <div className={cx('progress')} style={{ width: `${(recruiter?.points / 800) * 100}%` }}></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className={cx('stats-section')}>
          <div className={cx('stats-grid')}>
            ...
            <div className={cx('stat-card')}>
              ...
              <div className={cx('stat-info')}>
                <span className={cx('stat-value')}>{totalApplications7DaysAgo}</span>
        ...
        <div className={cx('company-info')}>
          <div className={cx('company-header')}>
            <div className={cx('company-logo')}>
              <img src={companyInfo?.logo || images.company_info} alt="Company Logo" />
            </div>
            <div className={cx('company-main-info')}>
              <h3>{companyInfo?.company_name || "Chưa cập nhật tên công ty"}</h3>
            ...
            <button onClick={() => setShowCompanyInfo(!showCompanyInfo)} className={cx('view-company-btn')}>
              {showCompanyInfo ? (
                <>
              ...
              <div className={cx('detail-grid')}>
                ...
                <div className={cx('detail-item')}>
                  ...
                  <div>
                    ...
                    <p>{new Date(companyInfo?.created_at).toLocaleDateString('vi-VN')}</p>
                  </div>
                </div>
                ...
                <div className={cx('detail-item')}>
                  ...
                  <div>
                    ...
                    <p>{new Date(companyInfo?.last_modified_at).toLocaleDateString('vi-VN')}</p>
                  </div>
                </div>
              </div>
            </div>
